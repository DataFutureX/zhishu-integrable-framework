...
public class AiConfig {
    ...
    private static final String SYSTEM_WITH_TOOLS = """
            ...
            - 「巡检计划」→ listInspectionPlans / getInspectionPlan（projectId 必填，先调 listProjects 获取）
            - 「巡检任务」→ listInspectionTasks / getInspectionTaskDetail（projectId 必填）
            - 「巡检异常」→ listOpenInspectionIssues（projectId 必填）
            - 「巡检摘要」→ getInspectionSummary（projectId 必填）
            - 调用巡检类工具前必须先调 listProjects 拿到 projectId 再传入，禁止省略或传 keyword 等不存在的参数
