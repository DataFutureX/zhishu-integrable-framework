...
import com.datafuturex.assistant.shared.vo.structured.AlarmSummary;
import com.datafuturex.assistant.shared.vo.structured.StationCompareResult;
import com.datafuturex.assistant.shared.vo.structured.TrendAnalysisResult;
...
public class AiChatServiceImpl implements AiChatService {
    ...
    public Flux<ServerSentEvent<String>> streamChat(ChatRequestDTO request) {
        ...
        if (looksLikeOnlineOverview(request.message())) {
            ...
            log.info("流式聊天短路在线概览 conversationId={}, items={}", conversationId, overview.items().size());
            return ChatSseSupport.toSseFlux(overview.markdown(), conversationId);
        }
    ...
    private ChatResponseVO buildOnlineStatusResponse(String conversationId) {
        ...
        // content 带完整 Markdown 表；structured 供前端 el-table 渲染
        return ChatResponseVO.of(
                overview.markdown(),
                aiModelConfigService.currentChatModel(),
                conversationId,
