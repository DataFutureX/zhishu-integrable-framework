...
CREATE TABLE IF NOT EXISTS ai_briefing_delivery (
    ...
    webhook_status   VARCHAR(16)  NOT NULL DEFAULT 'NONE',
    webhook_error    TEXT,
...
COMMENT ON COLUMN ai_briefing_delivery.email_status IS 'NONE | PENDING | SENT | SKIPPED | FAILED';
...
COMMENT ON COLUMN open_app.allowed_scopes IS 'JSON 数组：chat / knowledges / briefings 等';
