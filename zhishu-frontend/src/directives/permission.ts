...
function applyPermission(el: HTMLElement, binding: DirectiveBinding<PermissionValue>, userStore: Awaited<ReturnType<typeof loadUserStore>>) {
...
function checkPermission(el: HTMLElement, binding: DirectiveBinding<PermissionValue>) {
  void loadUserStore().then((userStore) => {
    applyPermission(el, binding, userStore)
  })
}
