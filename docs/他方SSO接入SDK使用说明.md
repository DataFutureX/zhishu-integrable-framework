...
**适用对象：** 万象、数智 IoT 等需「一键进入云起」的伙伴后端

本 SDK **只在他方（伙伴）侧使用**：用伙伴私钥签发短期 Ticket，拼装云起前端回调 URL。  
云起验签、换票、签发业务 JWT 仍由云起完成；**不要**把本 SDK 部署到云起服务器，也**不要**把私钥交给云起。
...
| 能力 | 说明 |
|------|------|
| 生成 RSA / SM2 密钥对 | 导出 PEM；公钥交云起登记，私钥自持 |
| 签发 Ticket | `alg=RS256` 或 `alg=SM2`（SM3withSM2） |
| 拼装回调 URL | `{YUNQI_WEB}/sso/callback?ticket=...&redirect=...` |
| ~~换票~~ | 不调用云起 API；由浏览器进回调页，或伙伴自行 `POST /api/v1/auth/sso/exchange` |
| ~~业务 JWT~~ | 不签发、不解析云起业务 Token |

---

## 2. 环境要求

- JDK **17+**（与云起后端 21 兼容；伙伴侧 17 即可）
- Maven 3.8+
...
rsa.writePublicKeyPem(Path.of("wanxiang-public.pem"));   // 交给云起
...
| 方法 | 说明 |
|------|------|
| `builder()` | 配置 issuer / kid / algorithm / 私钥 / 云起前端地址 |
...
1. **私钥不出伙伴机房**：禁止提交 git、禁止发聊天/工单附件、禁止放到云起机器。  
2. Ticket **TTL 建议 60 秒**，禁止做成会话级长 Token。  
3. Ticket **只能换票一次**（`jti` 防重放）；不要把同一 Ticket 缓存给多人用。  
4. `redirect` 只能是云起站内相对路径（以 `/` 开头）。  
5. 生产必须用 **HTTPS** 传 Ticket。  
6. 用户名必须与云起已开通账号一致；云起第一期**不会**自动建号。  
7. 禁止使用云起登录页 RSA 公钥或云起 `jwt.secret` 签发 Ticket。
...
| # | 检查项 | 通过标准 |
|---|--------|----------|
| 1 | 公钥已登记到云起 | `yunqi.sso.partners.<iss>` + `kid` |
| 2 | 云起 SSO 开关 | `yunqi.sso.enabled=true` |
| 3 | 测试账号已开通且启用 | `sys_user.username` 一致、`status=1` |
| 4 | SDK issuer / kid / alg 与云起一致 | 换票不再报「签名无效 / 未开通来源」 |
| 5 | 浏览器打开 callbackUrl | 进入云起业务页，地址栏无残留 ticket |
...
**Q: 报「票据签名无效」？**  
A: 核对公私钥是否一对、`alg` 与公钥类型是否匹配、云起 `kid` 是否指向正确 PEM。SM2 与 RSA 公钥不可混用。

**Q: 报「账号未开通」？**  
A: 在云起预建与 Ticket `username`/`sub` 相同的用户。
